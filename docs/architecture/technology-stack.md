...
- TypeBox/Ajv schemas for every HTTP/event boundary, generated OpenAPI/client
  types, `AsyncLocalStorage`, Biome, fast-check, Knip, markdownlint, Renovate,
  cross-platform CI, SBOMs, and signed artifacts remain target-state work;
- checked-in transactional migrations and the session-scoped FTS5 artifact
  repository are adopted;
